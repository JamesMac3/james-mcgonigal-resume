...
export default function DefenseHUD({ game }) {
  ...
  return <aside className={`defense-hud ${secured ? 'is-secured' : ''}`} aria-label="Core defense status">
    <div className="defense-hud-label"><i/>{['failing','shattering','lost'].includes(game.phase) ? 'CORE OFFLINE' : countdown ? 'TRACE COMPLETES IN' : secured ? 'ALL FIVE WAVES CLEARED' : `LEVEL ${game.level} / ${game.level === 5 ? 'DDOS ATTACK' : 'CORE DEFENSE'}`}</div>
    <div className="defense-number" aria-hidden="true">{countdown ? formatCountdown(game.remaining) : game.phase === 'detonating' ? '00:00:00' : score}</div>
