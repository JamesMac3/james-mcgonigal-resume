...
import { defenseReducer as reduce, initialDefenseState, formatCountdown, nearestMissile, INTEGRITY_TARGET, integrityFromScore } from '../src/defenseState.js';
...
test('intercepts score exactly once per HIT; impacts deduct 200 and zero integrity remains playable', () => {
...
test('first unprotected impact fails; pause freezes missiles; pools stay bounded', () => {
  ...
  try {
    ...
    assert.ok(sawImpact); assert.ok(sawRight); assert.equal(hits, 0); assert.equal(engine.failed, true);
...
test('five finite waves, curved aiming, and perfect defense reaches full integrity before DDOS', () => {
 ...
 try {
  ...
  let barrage=0;
  for(let i=0;i<6000;i++){engine.update(.1,playing);if(engine.level===5){barrage=Math.max(barrage,engine.missiles.filter(m=>m.active).length);assert.equal(integrityFromScore(engine.score),100);} for(const m of engine.missiles)if(m.active)engine.interceptMissile(m,false);}
  assert.deepEqual([...levels],[1,2,3,4,5]);assert.equal(impacts,0);assert.equal(engine.score,14500);assert.equal(completions,1);assert.ok(barrage>=10);assert.equal(engine.finished,true);
...
test('level 3 and 4 have exactly the requested small barrages', () => {
 ...
 try {
  ...
  for(const level of [3,4]) {
   ...
   assert.equal(barrages.length,level===3?5:3);
...
test('integrity threshold, loss sequence, scoring lock and reset',()=>{
 let state={...initialDefenseState(),phase:'playing'};
 assert.equal(state.integrity,0);
 state=reduce(state,{type:'HIT',praise:'NICE SHOT!'});assert.equal(state.praise,'NICE SHOT!');
 state=reduce(state,{type:'IMPACT'});assert.ok(state.integrity<=-1);assert.equal(state.phase,'failing');
 assert.equal(reduce(state,{type:'HIT'}),state);
 assert.equal(reduce(state,{type:'TICK',elapsed:5000,paused:true}),state);
 state=reduce(state,{type:'TICK',elapsed:1300});assert.equal(state.phase,'shattering');
 state=reduce(state,{type:'TICK',elapsed:1800});assert.equal(state.phase,'lost');
 state=reduce(state,{type:'ARM'});assert.equal(state.integrity,0);assert.equal(state.score,0);assert.equal(state.phase,'countdown');
 assert.equal(integrityFromScore(INTEGRITY_TARGET*100),100);
});
test('debris drifts inward using immutable shared geometry and all effects expire',()=>{
 const {engine}=testEngine();try{
 engine.update(.01,{...playing,phase:'countdown'});
 const burst=engine.burst(new THREE.Vector3(4,2,0),'#55bbff',false);
 engine.update(.5,{...playing,phase:'countdown'});
 const geometry = burst.points.geometry; const positions = geometry.attributes.position; const version = positions.version;
 const distance=()=>burst.points.position.length();const outward=distance();
 engine.update(.2,{...playing,phase:'countdown'});engine.update(.2,{...playing,phase:'countdown'});
 assert.ok(distance()<outward);
 for(let i=0;i<50;i++)engine.update(.1,{...playing,phase:'countdown'});
 assert.equal(engine.bursts.filter(b=>b.points.visible).length,0);
 for(let i=0;i<100;i++)engine.burst(new THREE.Vector3(), '#fff',false);
 assert.equal(engine.bursts.length,8);
 assert.ok(engine.bursts.every(b=>b.points.geometry === geometry));
 assert.equal(positions.count,24); assert.equal(positions.version,version);assert.equal(engine.popups.length,16);
 engine.update(.01,{...playing,phase:'shattering'});assert.ok(engine.bursts.every(b=>!b.absorb));
 engine.update(.01,{...playing,phase:'countdown'});assert.ok(engine.bursts.every(b=>!b.points.visible));
 }finally{engine.dispose();}
});
